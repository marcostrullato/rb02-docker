...
#screen -d -m /bin/bash -c '. /kinect_ws/devel/setup.bash; roslaunch freenect_launch freenect_throttle.launch rate:=5'
screen -d -m /bin/bash -c '. /kinect_ws/devel/setup.bash; roslaunch freenect_launch freenect.launch depth_registration:=true'
...
docker run --device=/dev/ttyUSB0 --device /dev/bus/usb:/dev/bus/usb --runtime nvidia -i -t --entrypoint /bin/bash --network host images.strullato.com:8080/rb02-all-in-one:1.4
